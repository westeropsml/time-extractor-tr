...
public class TemporalParser {
    ...
    public Temporal getHoliday(String holidayName) {
        ...
        if (holidayName.equalsIgnoreCase(Holidays.NEW_YEAR) || holidayName.equalsIgnoreCase(Holidays.NEW_YEAR4)) {
            return buildTemporalSpecial("01-01-00", "01-01-00");

        } else if (holidayName.equalsIgnoreCase(Holidays.HALLOWEEN)) {
            return buildTemporalSpecial("31-10-00", "31-10-00");
        }

        else if (holidayName.contains("Sevgililer Günü")) {
            return buildTemporalSpecial("14-02-00", "14-02-00");
        }
            ...
            return buildTemporalSpecial("25-12-00", "25-12-00");
        }
            ...
            return buildTemporalSpecial("04-07-00", "04-07-00");
        }
            ...
            return buildTemporalSpecial("20-01-00", "20-01-00");
        }
        ...
        /*
            ...
            return buildTemporalSpecial("11-11-00", "11-11-00");
        }

        else if (holidayName.equalsIgnoreCase(Holidays.COCUK_BAYRAM)) {
            return buildTemporalSpecial("23-04-00", "23-04-00");
        }

        else if (holidayName.equalsIgnoreCase(Holidays.ISCI_GUN)) {
            return buildTemporalSpecial("01-05-00", "01-05-00");
        }

        else if (holidayName.equalsIgnoreCase(Holidays.GENCLIK_BAYRAM)) {
            return buildTemporalSpecial("19-05-00", "19-05-00");
        }

        else if (holidayName.equalsIgnoreCase(Holidays.DEMOKRASI_GUN)) {
            return buildTemporalSpecial("15-07-00", "15-07-00");
        }

        else if (holidayName.equalsIgnoreCase(Holidays.ZAFER_BAYRAM)) {
            return buildTemporalSpecial("30-08-00", "30-08-00");
        }

        else if (holidayName.equalsIgnoreCase(Holidays.CUMHURIYET_BAYRAM)) {
            return buildTemporalSpecial("29-10-00", "29-10-00");
        }

        else if (holidayName.equalsIgnoreCase(Holidays.KURBAN_BAYRAM)) {
            /*
            ...
            */
            ...
            int year = dateNow.getYear() - 579;
            HijrahDate startDate = HijrahChronology.INSTANCE.date(year,12,10);
            HijrahDate endDate = HijrahChronology.INSTANCE.date(year,12,13);
            java.time.LocalDate startDate2 = IsoChronology.INSTANCE.date(startDate);
            String startDate3 = startDate2.format(DateTimeFormatter.ofPattern("dd-MM-00"));
            java.time.LocalDate endDate2 = IsoChronology.INSTANCE.date(endDate);
            String endDate3 = endDate2.format(DateTimeFormatter.ofPattern("dd-MM-00"));

            return buildTemporal(startDate3, endDate3);
        }

        else if (holidayName.equalsIgnoreCase(Holidays.RAMAZAN_BAYRAM)) {
            LocalDate dateNow = LocalDate.now();
            //Date dateNow = new Date();
            int year = dateNow.getYear() - 579;
            HijrahDate startDate = HijrahChronology.INSTANCE.date(year,10,1);
            HijrahDate endDate = HijrahChronology.INSTANCE.date(year,10,3);
            java.time.LocalDate startDate2 = IsoChronology.INSTANCE.date(startDate);
            String startDate3 = startDate2.format(DateTimeFormatter.ofPattern("dd-MM-00"));
            java.time.LocalDate endDate2 = IsoChronology.INSTANCE.date(endDate);
            String endDate3 = endDate2.format(DateTimeFormatter.ofPattern("dd-MM-00"));
    ...
    public Temporal getRelativeTemporalObjectByProperty(String property, LocalDateTime localDate) {
        ...
        if (property.equalsIgnoreCase("bugün")) {
            ...
            return TemporalObjectGenerator.generateTemporalTimeSpecial(Type.DATE, timeDate);
    ...
    public Temporal getDuration(String durationType, int durationLength) {
        ...
        if (durationType.equalsIgnoreCase("yıl") || durationType.equalsIgnoreCase("sene")) {
            duration = new Duration();
    ...
    public Temporal getRelativeDurationDate(String text, LocalDateTime dateTime) {
        ...
        Temporal temporal = TemporalObjectGenerator.generateTemporalTimeSpecial(Type.DATE, timeDate);
        return temporal;
    ...
    public Temporal getRelativeDurationDate2(String text, LocalDateTime dateTime) {
        ...
        int durationLength = TemporalBasicCaseParser.getIntFromBasicTerm(m.group(1));
        String durationType = m.group(14);
        if (dateTime == null) {
            dateTime = LocalDateTime.now();
        }
        if (durationType == null) {
    ...
    public Temporal getTimeOfDay(String timeOfDay) {
        ...
        if (timeOfDay.equalsIgnoreCase("öğleden önce") || timeOfDay.equalsIgnoreCase("öğleden önceleri")) {
    ...
    public Temporal getTemporalForEveryPeriod(String period) {
        ...
        else if (period.equalsIgnoreCase("saatlik") || period.equalsIgnoreCase("saat")) {
    ...
    private Temporal buildTemporal(String start, String end) {
        ...
        Date endDate = new Date(Integer.parseInt(startEl[2]), Integer.parseInt(endEl[1]), Integer.parseInt(endEl[0]));
    ...
    private Temporal buildTemporalSpecial(String start, String end) {
        String startEl[] = start.split("-");
        String endEl[] = end.split("-");
        ...
        //System.out.println(formatter.format(todayy));

        int dayToday = todayy.getDayOfMonth();
        int monthToday = todayy.getMonthOfYear();
        int yearToday = todayy.getYear();
        int monthDay = (monthToday*100) + dayToday;
        int monthDayStart = (Integer.parseInt(startEl[1])*100) + Integer.parseInt(startEl[0]);

        TimeDate start_date = new TimeDate();
        TimeDate end_date = new TimeDate();

        if (monthDayStart >= monthDay){
            Date startDate = new Date(yearToday, Integer.parseInt(startEl[1]), Integer.parseInt(startEl[0]));
            Date endDate = new Date(yearToday, Integer.parseInt(endEl[1]), Integer.parseInt(endEl[0]));
            start_date.setDate(startDate);
            end_date.setDate(endDate);
        }
        else{
            Date startDate = new Date(yearToday+1, Integer.parseInt(startEl[1]), Integer.parseInt(startEl[0]));
            Date endDate = new Date(yearToday+1, Integer.parseInt(endEl[1]), Integer.parseInt(endEl[0]));
            start_date.setDate(startDate);
            end_date.setDate(endDate);
        }
        return new Temporal(start_date, end_date);
