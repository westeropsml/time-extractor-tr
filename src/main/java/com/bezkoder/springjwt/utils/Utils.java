...
public class Utils {
    ...
    // flags
    public static Matcher getMatch(String regexRule, String text) {
        Pattern p = Pattern.compile(regexRule, Pattern.CASE_INSENSITIVE);
...
//  covert time special = Türkçeye çevrildi.
    public static int convertTimeSpecial(int time, String aMPm) {
        aMPm=aMPm.toLowerCase();
        if (aMPm.equalsIgnoreCase("öğleden sonra") || aMPm.equalsIgnoreCase("akşam")||aMPm.equalsIgnoreCase("öğlen")|| aMPm.equalsIgnoreCase("öğle")|| aMPm.equalsIgnoreCase("ös")|| aMPm.equalsIgnoreCase("ö.s")|| aMPm.equalsIgnoreCase("ö.s.")) {
            if (time >= 12 && time < 24) {
                return time;
            } else if (time == 24) {
                return 0;
